exports.run = async (client, reaction, user) => {
	...
	try {
		await member.roles.remove(role.id);
  		return console.log(`[REACTION_ROLES]`, `${member.displayName} (${user.id}) devolveu o cargo "${role.name}"`);
	} catch (err) {
		console.log(`[REACTION_ROLES]`, `não foi possível retirar o cargo "${role.name}" de ${user.tag} (${user.id})`);
