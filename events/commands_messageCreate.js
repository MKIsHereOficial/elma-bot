...
exports.run = async (client, message) => {
	if (![...client.creators, '941507039346196572'].includes(message.author.id))
		return;
	if (
		[
			`<@!${client.user.id}> registro`,
			`<@${client.user.id}> registro`,
		].includes(message.content.toLowerCase())
	)
		return client.emit('buildRegister', message);
	if (
		[
			`<@!${client.user.id}> cores`,
			`<@${client.user.id}> cores`,
		].includes(message.content.toLowerCase())
	)
		return client.emit('buildColorRoles', message);
...
`
		...
		return message
			.reply({ content: `${message.member}`, embeds: [embed] })
			.then((msg) => {
				message.delete();
				return setTimeout(() => {
					msg.delete();
